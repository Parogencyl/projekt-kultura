...
<section class="container mb-5 mt-5">
    ...
    <div class="alert alert-danger alert-block col-12 my-5">
    ...
    <div class="alert alert-success alert-block col-12 my-5">
        <button type="button" class="close" data-dismiss="alert">×</button>
        <strong>{{ $message }}</strong>
    </div>
    @endif
    ...
    <div class="row justify-content-center">
        ...
        <form action="{{ url('/zakup/sprawdzenie') }}" method="POST" class="form col-12 col-md-10 col-xl-9">
            @csrf

            <input type="hidden" name="name_of_course" value="{{ $nazwa }}">
            <input type="hidden" name="price" value="{{ $kurs->cena }}" id="priceToPay">
            ...
            <div class="form-group mb-3 ml-2">
                <div>
                    <input id="email" type="email" class="form-control @error('email') is-invalid @enderror"
                        name="email" required autocomplete="email" value="{{ old('email') }}" placeholder="E-mail *">
                    @error('email')
            ...
            <div class="form-group mb-3 ml-2">
                <div>
                    <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                        required autocomplete="billing given-name" value="{{ old('name') }}" placeholder="Imię *">
                    @error('name')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group mb-3 ml-2">
                <div>
                    <input id="phone" type="text" class="form-control @error('phone') is-invalid @enderror" name="phone"
                        required autocomplete="phone" value="{{ old('phone') }}" placeholder="Telefon *">
                    @error('phone')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3 ml-2">
                    <div>
                        <input id="street" type="text" class="form-control @error('street') is-invalid @enderror"
                            name="street" autocomplete="street" value="{{ old('street') }}" placeholder="Ulica *">
                        @error('street')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group mb-3 ml-2">
                    <div>
                        <input id="numberOfFlat" type="text"
                            ...
                            autocomplete="numberOfFlat" value="{{ old('numberOfFlat') }}"
                ...
                <div class="form-group mb-3 ml-2">
                    <div>
                        <input id="city" type="text" class="form-control @error('city') is-invalid @enderror"
                            name="city" autocomplete="city" value="{{ old('city') }}" placeholder="Miejscowość *">
                        @error('city')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-group mb-3 ml-2">
                    <div>
                        <input id="zip" type="text" class="form-control @error('zip') is-invalid @enderror" name="zip"
                            autocomplete="zip" value="{{ old('zip') }}" placeholder="Kod pocztowy *">
                        @error('zip')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="form-check mb-3 ml-4">
                <input type="checkbox" class="form-check-input @error('regulamin') is-invalid @enderror" id="regulamin"
